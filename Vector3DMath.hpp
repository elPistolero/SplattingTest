...
struct Vector {
        ...
        Vector(const float s = 0): x(s), y(s), z(s) {
        }
...
inline istream& operator >> (istream& is, Vector& v) {
        is >> ws;
        if(is.peek() != '(') is.clear(std::ios::failbit);
        is.get();
        is >> v.x >> ws;
        if(is.peek() != ',') is.clear(std::ios::failbit);
        is.get();
...
struct Matrix {
        ...
        Matrix(const float s = 1): a(s,0,0), b(0,s,0), c(0,0,s) {
        }

        Matrix(const float s, const float t, const float u): a(s,0,0), b(0,t,0), c(0,0,u) {
        }

        Matrix(const Vector& a, const Vector& b, const Vector& c): a(a), b(b), c(c) {
        }
        ...
        Matrix transpose() const {
                return Matrix(Vector(a.x, b.x, c.x), Vector(a.y, b.y, c.y), Vector(a.z, b.z, c.z));
        }

        Matrix inverse() const {
                Matrix adjugate;
                adjugate.a.x = b.y * c.z - c.y * b.z;
                adjugate.a.y = b.z * c.x - c.z * b.x;
                adjugate.a.z = b.x * c.y - c.x * b.y;
                adjugate.b.x = a.z * c.y - c.z * a.y;
                adjugate.b.y = a.x * c.z - c.x * a.z;
                adjugate.b.z = a.y * c.x - c.y * a.x;
                adjugate.c.x = a.y * b.z - b.y * a.z;
                adjugate.c.y = a.z * b.x - b.z * a.x;
                adjugate.c.z = a.x * b.y - b.x * a.y;
                const float determinant = a.x * adjugate.a.x + b.x * adjugate.b.x + c.x * adjugate.c.x;
